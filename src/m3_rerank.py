...
class FlashrankReranker:
    ...
    def rerank(self, query: str, documents: list[dict], top_k: int = RERANK_TOP_K) -> list[RerankResult]:
        ...
        doc_by_text = {d["text"]: d for d in documents}
        ranked = sorted(results, key=lambda r: r["score"], reverse=True)[:top_k]
        return [
            RerankResult(text=r["text"], original_score=doc_by_text.get(r["text"], {}).get("score", 0.0),
                        rerank_score=float(r["score"]), metadata=doc_by_text.get(r["text"], {}).get("metadata", {}), rank=i)
