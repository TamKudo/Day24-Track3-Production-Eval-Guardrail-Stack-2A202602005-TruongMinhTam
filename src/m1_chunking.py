...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i, group in enumerate(groups):
        chunks.append(Chunk(text=" ".join(group), metadata={**metadata, "chunk_index": i, "strategy": "semantic"}))
    return chunks
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    def _flush():
        ...
        chunks.append(Chunk(
            text=chunk_text,
            metadata={**metadata, "chunk_index": len(chunks), "section": current_header.lstrip("# ").strip(),
                      "strategy": "structure"},
        ))
