...
def judge_single_answer(question: str, answer: str) -> int:
    """Judge một câu trả lời đơn là tốt (1) hay xấu (0), dùng cho Cohen's κ."""
    from openai import OpenAI
    client = OpenAI(api_key=OPENAI_API_KEY)
    prompt = f'''Bạn là expert đánh giá câu trả lời chính sách nhân sự (HR policy) tiếng Việt.
...
Đánh giá câu trả lời có chính xác và đầy đủ theo chính sách công ty hiện hành không
(chú ý: nếu có nhiều phiên bản chính sách, chỉ phiên bản mới nhất/hiện hành mới đúng).
Trả lời JSON (chỉ JSON, không text khác):
...
'''
    ...
    return int(json.loads(resp.choices[0].message.content)["label"])
...
if __name__ == "__main__":
    ...
    # --- Bias report: run pairwise + swap on a handful of pairs ---
    # Trộn vị trí câu đúng/dài giữa A và B để tránh thiên lệch nhân tạo trong bộ demo.
    pairwise_pairs = [
        (q, a_a, a_b),  # A đúng+dài hơn, B sai+ngắn
        ("Thưởng Tết tối thiểu cho nhân viên chính thức là bao nhiêu?",
         "Thưởng Tết.",  # A sai+ngắn
         "Nhân viên được thưởng Tết tối thiểu 1 tháng lương theo chính sách hiện hành."),  # B đúng+dài
        ("Nhân viên thử việc có được nghỉ phép năm không?",
         "Nhân viên thử việc không được nghỉ phép năm và phải xin nghỉ không lương nếu cần.",  # A đúng+dài
         "Có, thử việc vẫn được nghỉ phép như nhân viên chính thức."),  # B sai+ngắn hơn
        ("Muốn mua thiết bị trị giá 55 triệu cần ai phê duyệt?",
         "Cần Giám đốc phòng ban phê duyệt.",  # A sai+ngắn
         "Cần CEO phê duyệt vì vượt ngưỡng 50 triệu do chính sách mua sắm quy định."),  # B đúng+dài
        ("Nhân viên Manager có thể dùng VPN cá nhân khi WFH không?",
         "Được, miễn là đảm bảo kết nối an toàn.",  # A sai+ngắn
         "Không, VPN cá nhân bị cấm; phải dùng VPN WireGuard của công ty."),  # B đúng+dài
    ]
    judge_results = [swap_and_average(qq, aa, bb) for qq, aa, bb in pairwise_pairs]
